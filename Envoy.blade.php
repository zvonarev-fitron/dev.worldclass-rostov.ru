...
@setup
    ...
    $git_hub = 'git@github.com:zvonarev-fitron/dev.worldclass-rostov.ru.git';
    $site = 'dev.worldclass-rostov.ru';
...
@task('dump', ['on' => 'web'])
    cd {{ $current }}
    PGPASSWORD="{{ $db_password }}" pg_dump -U {{ $db_user }} -n {{ $db_schema }} dev_wc > {{ $db_name }}_{{ $new_dir }}.sql
@endtask
...
@task('copy', ['on' => 'web'])
    cp -r {{ $current }}/storage/app/public/svg/ {{ $release }}/storage/app/public/
    cp -r {{ $current }}/storage/app/public/images/ {{ $release }}/storage/app/public/
    cp -r {{ $current }}/storage/app/public/uploads/ {{ $release }}/storage/app/public/
@endtask
